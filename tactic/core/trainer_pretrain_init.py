...
from tactic.core.collator import CollatorWithPadding
from tactic.data.dataset_synthetic import SyntheticDataset
from tactic.utils.set_seed import seed_worker
...
def prepare_ddp_model(cfg: ConfigPretrain, model: torch.nn.Module) -> torch.nn.Module:
    ...
    if cfg.use_ddp:
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[cfg.device], find_unused_parameters=False)
        model.train_forward = lambda *args, **kwargs: model.module.train_forward(*args, **kwargs)
