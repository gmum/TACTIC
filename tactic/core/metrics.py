...
class MetricsTraining():
    ...
    def update(self, pred: torch.Tensor, target: torch.Tensor):
        ...
        correct = pred_.eq(target)
        self._correct += pred_.eq(target).sum().item()

        self._total += target[target != -100].shape[0]

        # Per-class accuracy tracking
        for t, c in zip(target.tolist(), correct.tolist()):
            self._per_class_total[t] += 1
            if c:
                self._per_class_correct[t] += 1

    def reset(self):
        self._loss = 0
        self._correct = 0
        self._total = 0
        self._per_class_correct = defaultdict(int)
        self._per_class_total = defaultdict(int)
    ...
    def per_class_accuracy(self):
        """Returns {class_id: accuracy} dictionary."""
        return {
            f"train/acc_for_{cls}_cls": self._per_class_correct[cls] / total
            for cls, total in self._per_class_total.items()
            if total > 0
        }
...
class MetricsValidation():
    ...
    def update(self, pred: torch.Tensor, target: torch.Tensor):
        ...
        correct_vec = pred_.eq(target)
        for t, c in zip(target.tolist(), correct_vec.tolist()):
            per_class_total[t] += 1
            if c:
                per_class_correct[t] += 1

        # Convert to fixed dict of {class: acc}
        per_class_acc = {
            f"val/acc_for_{cls}_cls": per_class_correct[cls] / total_count
            for cls, total_count in per_class_total.items()
        }

        self._per_class_acc.append(per_class_acc)
